...
export const AI_PROMPT = 'Generate {preferences} Travel Plan from {country} to {location} as a {muslim}, for {number_of_days} Days for {travel_companions} with a {budget} budget with a Flight details, Flight Price with Booking url, Hotels options list with HotelName, Hotel address, Price, hotel image url, geo coordinates, rating, descriptions and Places to visit nearby with placeName, Place Details, Place Image Url, Geo Coordinates, ticket Pricing, Time travel each of the location for {number_of_days} days with each day plan with best time to visit in JSON format'; 
